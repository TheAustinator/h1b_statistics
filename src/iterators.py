...
class MultiFileCounter:
    ...
    def add_constraint(self, alias, cmp, val):
        ...
        Args:
            alias (str): alias for column names to be constrained. Must
                ...
                files
            comparator: operator to compare row to value at column specified by
                alias
            val: value against which to compare row at column

        Example Usage:
            >>>mfc.add_constraint('population', op.gt, 10,000),
            >>>mfc.add_constraint('latitude', 'op.lt, 40),
            >>>mfc.add_constraint('region', op.eq, 'europe'),
        """
        constraint = self.Constraint(alias, cmp, val)
    ...
    def constraints_satisfied(self, row):
        ...
        Args:
            row (namedtup): row containing only columns of interest
